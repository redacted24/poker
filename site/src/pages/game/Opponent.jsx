...
const Opponent = ({ player, isCurrentPlayer }) => {
    const positionTag = () => {
        switch (player.position) {
            case 0:
                return (
                    <img className="position-tag" src="/positions/dealer.png" />
                );
            case 1:
                return (
                    <img
                        className="position-tag"
                        src="/positions/small-blind.png"
                    />
                );
            case 2:
                return (
    ...
    const previousStep = () => {
        if (player.previous_step) {
            if (player.previous_step.length == 2) {
                return (
                    <p className="previous-step">
                        {player.previous_step.join(" ") + " $"}
                    </p>
                );
            } else {
                return (
                    <p className="previous-step">{player.previous_step[0]}</p>
                );
            }
        } else {
